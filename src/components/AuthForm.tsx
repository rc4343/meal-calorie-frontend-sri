  ...
  async function onSubmit(data: Record<string, any>) {
    ...
    try {
      const res = isRegister
        ? await registerUser(data as unknown as RegisterFormData)
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Button type="button" variant="outline" className="w-full" onClick={() => toast.info(t("auth.sso.comingSoon"))}>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-card px-2 text-muted-foreground">{t("auth.sso.divider")}</span>
              </div>
            </div>
